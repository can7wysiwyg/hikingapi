...
class NotificationService {
    static async sendFCMNotification(receiver, sender, content, conversationId, messageId, senderId) {
        try {
            ...
            // Generate an FCM access token (directly here instead of importing)
            const token = await admin.auth().createCustomToken(receiverId); // Generates token for the receiver
